...
Time Tracker 支持基于阿里云 OSS 的多端同步功能，允许在手机和电脑之间自动同步数据。

### 核心特性

- ✅ **本地优先**：所有数据首先保存在本地，离线也能正常使用
- ✅ **自动同步**：每 10 分钟自动同步一次，也可手动触发
...
- 应用启动时自动同步一次
- 每 10 分钟自动同步
- 可在代码中修改间隔（`App.tsx` 中的 `startAutoSync(10)`）

### 查看同步状态

- **同步中**：按钮显示 loading 动画
- **同步成功**：显示绿色勾号，提示 `同步成功 (↑N ↓M)`
  - ↑N：上传的操作数
  - ↓M：下载的操作数
- **同步失败**：显示红色叉号，并提示错误信息
...
- `src/services/syncEngine.ts` - 同步引擎核心逻辑
- `src/stores/syncStore.ts` - 同步状态管理
- `src/components/SyncButton/SyncButton.tsx` - 同步按钮 UI
...
// 自动同步
const cleanup = startAutoSync(10); // 10分钟
```
