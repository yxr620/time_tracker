...
│   │   ├── common/           # 通用 UI 组件
│   │   │   ├── WheelTimePicker.tsx
│   │   │   └── SyncToastListener.tsx  # 全局同步 toast 监听
│   │   ├── TimeTracker/
...
│   │   │   └── TimeInjectionMatrix.tsx  # 时间注入矩阵（月历热力图）
│   │   ├── RecordsPage/
...
│   │   └── aiStore.ts        # AI 配置 + 对话状态 store
│   ├── services/             # 业务逻辑
│   │   ├── db.ts             # IndexedDB 数据库定义
│   │   ├── syncDb.ts         # 带同步操作记录的数据库封装
│   │   ├── syncEngine.ts     # 同步引擎（压缩/快照/增量/全量/清理）
│   │   ├── export.ts         # 数据导入导出
│   │   ├── oss.ts            # 阿里云 OSS 操作（oplog/snapshot/分页/批量删除）
│   │   ├── syncToast.ts      # 全局同步 toast 事件通道
│   │   ├── syncDebugTools.ts # 同步调试工具
│   │   ├── goalSuggester.ts  # 目标建议
│   │   ├── metadataPredictor.ts # 元数据预测
│   │   ├── analysis/         # 数据分析处理
│   │   │   ├── processor.ts
│   │   │   ├── goalCluster.ts
│   │   │   └── goalAnalysisProcessor.ts
│   │   └── ai/               # AI 时间助手
│   │       ├── toolCallEngine.ts    # 工具调用循环引擎
