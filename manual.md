...
**日期同步说明**：记录页面、时间轴、目标页面共享同一选中日期；在任意页面切换日期，其他页面保持一致，不会跳回今天。
...
- Tailwind CSS（样式方案）
- Recharts（图表库，桌面端数据分析）
- Dexie.js (IndexedDB)
- Zustand (状态管理)
- Day.js + date-fns (时间处理)
- Vite (构建工具)
- Capacitor (原生应用打包)
- Electron (桌面端打包)
...
│   ├── EntryList/        # 记录列表
│   ├── GoalManager/      # 目标管理
│   ├── Dashboard/        # 数据分析看板（桌面端）
│   │   ├── Dashboard.tsx
│   │   └── Dashboard.css
│   ├── TrendPage/        # 趋势分析页面（桌面端）
│   │   ├── TrendPage.tsx
│   │   └── TrendPage.css
│   ├── GoalAnalysisPage/ # 目标深度分析页面（桌面端）
│   │   ├── GoalAnalysisPage.tsx
│   │   └── GoalAnalysisPage.css
│   └── Desktop/          # 桌面端专属组件
│       └── DesktopSidebar.tsx  # 侧边栏导航
├── types/
