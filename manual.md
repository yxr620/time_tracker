...
这是一款**个人时间追踪工具**，帮助你线性记录每天不同时刻在做什么事情，设置每日目标，并统计各目标的时间投入情况。
...
> 💡 该功能仅在桌面端可用，点击侧边栏的「AI」进入。需要配置 AI 服务商的 API Key。

### 1. 功能概述

AI 时间助手是一个对话式界面，你可以用自然语言提问关于你时间记录的任何问题。AI 会自动检索你的时间数据并给出分析回答。
...
**快捷问题：**
- 首次进入会显示预设的快捷问题按钮，点击即可提问
- 对话中也会在底部显示常用快捷问题

**自由提问：**
- 在底部输入框中输入问题，按 Enter 或点击发送按钮
- 支持 Shift+Enter 换行
- 发送过程中可以点击停止按钮中断生成
...
**服务商选择：**
支持多个 AI 服务商，所有服务商均兼容 OpenAI 协议：
- 通义千问
- Gemini
- 智谱 GLM
- Kimi
- MiniMax
- OpenAI
- 自定义（可接入任何 OpenAI 兼容的接口）

**配置项：**
- **API Key**：密钥仅存储在本地，不会上传到任何服务器
- **模型选择**：每个服务商提供多个模型可选
- **高级设置**：可自定义 Base URL，用于接入本地模型（如 Ollama）或其他代理

### 4. 工作原理

提问后会经过以下阶段（界面上有进度指示）：
1. **解析时间范围**：理解你问题中的时间表达（如"上周"、"最近7天"）
2. **检索数据**：从本地数据库中获取对应时间范围的记录
3. **生成回答**：AI 基于你的数据生成分析回答
